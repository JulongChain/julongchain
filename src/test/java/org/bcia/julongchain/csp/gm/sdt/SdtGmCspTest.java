...
public class SdtGmCspTest {
    ...
    public void testKeyGen() {
        ...
        System.out.println("============= SdtGmCsp keyGen test =============");
        int caseIndex = 1;
    ...
    public void testKeyImport() {
        System.out.println("============= SdtGmCsp keyImport test =============");
        int caseIndex = 1;
        KeysStore keysStore = new KeysStore();
        ...
        try {
            ...
            System.out.println("[ output ] SM2 private key(raw data) : " + Convert.bytesToHexString(sm2RawDataPrivateKey));
            IKey sm2PrivateKeyEphemeral = csp.keyImport(sm2RawDataPrivateKey, new SM2PrivateKeyImportOpts(true));
            System.out.println("[ output ] SM2 private key(from memory) : " + Convert.bytesToHexString(sm2PrivateKeyEphemeral.toBytes()));
        ...
        try {
            ...
            System.out.println("[ output ] SM2 public key(raw data) : " + Convert.bytesToHexString(sm2RawDataPublicKey));
            IKey sm2PublicKeyEphemeral = csp.keyImport(sm2RawDataPublicKey, new SM2PublicKeyImportOpts(true));
            System.out.println("[ output ] SM2 public key(from memory) : " + Convert.bytesToHexString(sm2PublicKeyEphemeral.toBytes()));
        ...
        try {
            ...
            System.out.println("[ output ] SM4 key(raw data) : " + Convert.bytesToHexString(sm4RawDataKey));
            IKey sm4KeyEphemeral = csp.keyImport(sm4RawDataKey, new SM4KeyImportOpts(true));
            System.out.println("[ output ] SM4 key(from memory) : " + Convert.bytesToHexString(sm4KeyEphemeral.toBytes()));
    ...
    public void testGetKey() {
        System.out.println("============= SdtGmCsp getKey test =============");
        int caseIndex = 1;
        ...
        try {
            ...
            if(null != sm2PrivateKey) {
                System.out.println("[ output ] SM2 private key : " + Convert.bytesToHexString(sm2PrivateKey.toBytes()));
            } else {
        ...
        try {
            ...
            if(null != sm2PublicKey) {
                System.out.println("[ output ] SM2 public key : " + Convert.bytesToHexString(sm2PublicKey.toBytes()));
            } else {
    ...
    public void testHash() {
        ...
        for(int index = 0; index < testMessageLen.length; index ++) {
            try {
                ...
                byte[] digest = csp.hash(msg, new SM3HashOpts());
    ...
    public void testSign() {
        ...
        byte[] privateKey = {(byte)0x39, (byte)0x45, (byte)0x20, (byte)0x8F,
        ...
        byte[] publicKey = {(byte)0x09, (byte)0xF9, (byte)0xDF, (byte)0x31,
        ...
        try {
            IKey sm2Key = new SM2Key(new SM2KeyPair(publicKey, privateKey));
            byte[] signature = csp.sign(sm2Key, digest, new SM2SignerOpts());
            System.out.println("[ input ] SM2 private key : " + Convert.bytesToHexString(privateKey));
            System.out.println("[ input ] digest : " + Convert.bytesToHexString(digest));
            if(null != signature) {
    ...
    public void testVerify() {
        ...
        byte[] signature = {(byte)0x25, (byte)0x9E, (byte)0xE0, (byte)0x26,
        ...
        try {
            IKey sm2Key = new SM2Key(new SM2KeyPair(publicKey, privateKey));
            boolean result = csp.verify(sm2Key, signature, digest, new SM2SignerOpts());
            System.out.println("[ input ] SM2 public key : " + Convert.bytesToHexString(publicKey));
            System.out.println("[ input ] digest : " + Convert.bytesToHexString(digest));
            ...
            if(result) {
                ...
                System.out.println("[ output ] failed verifying the signature");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void testEncrypt() {
        ...
        int[] testPlainTextLen = {1, 16, 240, 2048};
        int plainTextLen = 0;
        byte[] key = {(byte)0x01, (byte)0x23, (byte)0x45, (byte)0x67,
                (byte)0x89, (byte)0xab, (byte)0xcd, (byte)0xef,
                (byte)0xfe, (byte)0xdc, (byte)0xba, (byte)0x98,
                (byte)0x76, (byte)0x54, (byte)0x32, (byte)0x10 };
        System.out.println("[ input ] SM4 key : " + Convert.bytesToHexString(key));
        for(int index = 0; index < testPlainTextLen.length; index ++) {
            try {
                plainTextLen = testPlainTextLen[index];
                byte[] plainText = new byte[plainTextLen];
                for(int i = 0; i < plainTextLen; i++) {
                    plainText[i] = (byte)(i+1);
                }
                System.out.println("\n**** case " + caseIndex++ + ": plain text length = "+ plainTextLen +"  ****");
                IKey sm4Key = new SM4Key(key);
                byte[] cipherText = csp.encrypt(sm4Key, plainText, new SM4EncrypterOpts());
                System.out.println("[ input ] plain text : " + Convert.bytesToHexString(plainText));
                if(null != cipherText) {
                    System.out.println("[ output ] cipher text : " + Convert.bytesToHexString(cipherText));
                    System.out.println("[ output ] cipher text length : " + cipherText.length);
                } else {
    ...
    public void testDecrypt() {
        ...
        byte[] key = {(byte)0x01, (byte)0x23, (byte)0x45, (byte)0x67,
                ...
                (byte)0x76, (byte)0x54, (byte)0x32, (byte)0x10 };
        System.out.println("[ input ] SM4 key : " + Convert.bytesToHexString(key));
        String[] cipherTextHexList = {"62a1af3b5d325afecec9a848f97d3be9",
        ...
        for(int index = 0; index < cipherTextHexList.length; index ++) {
            try {
                String cipherTextStr = cipherTextHexList[index];
                byte[] cipherText = Hex.decode(cipherTextStr);
                System.out.println("\n**** case " + caseIndex++ + ": cipher text length = "+ cipherText.length +"  ****");
                IKey sm4Key = new SM4Key(key);
                byte[] plainText = csp.decrypt(sm4Key, cipherText, new SM4DecrypterOpts());
