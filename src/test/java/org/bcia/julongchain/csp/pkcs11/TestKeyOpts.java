...
public class TestKeyOpts {
	...
	public void test1() {
		...
		try {
			...
			Assert.assertNotNull(bytehash1);
	...
	public void test4() {
		try {
			...
			Assert.assertTrue(rv1);
			boolean rv2 = compereByteArray(data,data1);
			Assert.assertTrue(rv2);
			boolean rv = compereByteArray(input2.getBytes(),data);
			Assert.assertTrue(rv);
	...
	public void test8() {
		...
		try {
			...
			byte[] ciphertext1 = cipher.doFinal(input2.getBytes());
			Assert.assertNotNull(ciphertext1);
